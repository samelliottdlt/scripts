...
export function updateShell(): void {
  ...
  const comment =
    "# Command added for binary shortcut - https://github.com/samelliottdlt/scripts";

  // Define the shell configuration file path based on the user's shell
  const shell = os.userInfo().shell;
  const rcFilePath = shell.includes("zsh")
  ...
  console.log(
    "Shell configuration updated successfully. You might need to restart your terminal for the changes to take effect."
  );
}
